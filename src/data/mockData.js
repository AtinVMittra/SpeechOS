export const patients = [
  {
    ...
    sessionPlan: {
      ...
      patientId: 'p1',
      sessionFocus: 'Drill /s/ and /z/ production in initial and final word positions using structured games.',
      totalDurationMinutes: 45,
      blocks: [
        { phase: 'Warm-Up', durationMinutes: 5, activity: 'Mirror Warm-Up', description: 'Quick mirror check — make /s/ sound 5 times, watch tongue placement.', materials: 'Mirror' },
        { phase: 'Targeted Drill', durationMinutes: 15, activity: 'Flashcard Drill', description: 'Work through /s/ word cards, mark correct/incorrect each trial.', materials: 'Flashcard game below' },
        { phase: 'Generalization Activity', durationMinutes: 15, activity: 'Minimal Pairs + Word Sort', description: 'Discrimination and categorization games for /s/ vs similar sounds.', materials: 'Games below' },
        { phase: 'Wrap-Up', durationMinutes: 10, activity: 'Sentence Practice', description: 'Use 2 words from the session in a sentence. Give feedback on /s/ accuracy.', materials: 'None' },
      ],
      exercises: [
        {
          id: 'demo-ex-1',
          gameType: 'flashcard',
          title: '/s/ Word Flashcard Drill',
          instructions: 'Show each card. Emma says the word; mark correct if /s/ is produced accurately.',
          targetTrials: 10,
          targetAccuracy: '80%',
          ageNote: 'Large text and emojis keep a 7-year-old engaged.',
          cards: [
            { word: 'sun', emoji: '☀️', targetSound: '/s/ initial', hint: 'tongue behind top teeth' },
            { word: 'sock', emoji: '🧦', targetSound: '/s/ initial', hint: 'start with /s/, not /th/' },
            { word: 'snake', emoji: '🐍', targetSound: '/s/ initial', hint: 'hold the /s/ sound' },
            { word: 'spoon', emoji: '🥄', targetSound: '/s/ blend', hint: '/s/ then /p/ quickly' },
            { word: 'star', emoji: '⭐', targetSound: '/s/ blend', hint: '/s/ + /t/ together' },
            { word: 'bus', emoji: '🚌', targetSound: '/s/ final', hint: 'end with a clear /s/' },
            { word: 'ice', emoji: '🧊', targetSound: '/s/ final', hint: 'soft /s/ at the end' },
            { word: 'house', emoji: '🏠', targetSound: '/s/ final', hint: 'don\'t drop the /s/' },
            { word: 'smile', emoji: '😊', targetSound: '/s/ blend', hint: '/s/ + /m/ smoothly' },
            { word: 'slide', emoji: '🛝', targetSound: '/s/ blend', hint: '/s/ + /l/ — feel the /l/' },
          ],
        },
        {
          id: 'demo-ex-2',
          gameType: 'minimal-pairs',
          title: '/s/ vs /th/ Discrimination',
          instructions: 'Say either word aloud; Emma taps which word she heard. Vary the order randomly.',
          targetTrials: 8,
          targetAccuracy: '75%',
          ageNote: 'Two large buttons make it easy for a 7-year-old to respond quickly.',
          pairs: [
            { target: 'sip', foil: 'tip', targetSound: '/s/ vs /t/' },
            { target: 'sea', foil: 'tea', targetSound: '/s/ vs /t/' },
            { target: 'sink', foil: 'think', targetSound: '/s/ vs /th/' },
            { target: 'sing', foil: 'thing', targetSound: '/s/ vs /th/' },
            { target: 'sick', foil: 'thick', targetSound: '/s/ vs /th/' },
            { target: 'sought', foil: 'thought', targetSound: '/s/ vs /th/' },
            { target: 'sum', foil: 'thumb', targetSound: '/s/ vs /th/' },
            { target: 'seem', foil: 'team', targetSound: '/s/ vs /t/' },
          ],
        },
        {
          id: 'demo-ex-3',
          gameType: 'word-sort',
          title: 'Initial vs. Final /s/ Sort',
          instructions: 'Emma taps a word, then taps its category. Check answers together at the end.',
          targetTrials: 12,
          targetAccuracy: '80%',
          ageNote: 'Category sorting is a natural game for school-age children.',
          categories: ['Initial /s/', 'Final /s/'],
          words: [
            { word: 'sun', category: 'Initial /s/' },
            { word: 'bus', category: 'Final /s/' },
            { word: 'sock', category: 'Initial /s/' },
            { word: 'ice', category: 'Final /s/' },
            { word: 'sit', category: 'Initial /s/' },
            { word: 'house', category: 'Final /s/' },
            { word: 'seven', category: 'Initial /s/' },
            { word: 'mouse', category: 'Final /s/' },
            { word: 'soup', category: 'Initial /s/' },
            { word: 'goose', category: 'Final /s/' },
            { word: 'salt', category: 'Initial /s/' },
            { word: 'grass', category: 'Final /s/' },
          ],
        },
      ],
    },
