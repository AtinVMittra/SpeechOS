...
export default function PatientBrief({ patient, onNext }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Evaluation Outcome */}
      {patient.evaluationOutcome && (
        <div className="border border-violet-200 rounded-xl overflow-hidden">
          <div className="bg-gradient-to-r from-violet-600 to-violet-500 px-4 py-3 flex items-center gap-2">
            <svg className="w-4 h-4 text-violet-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
            </svg>
            <h3 className="text-sm font-semibold text-white">Evaluation Outcome</h3>
          </div>
          <div className="p-4 bg-white space-y-4">
            <p className="text-sm text-slate-700 leading-relaxed">{patient.evaluationOutcome.summary}</p>
            <div>
              <h4 className="text-xs font-semibold text-violet-700 uppercase tracking-wider mb-2">Key Areas for Improvement</h4>
              <ul className="space-y-1">
                {patient.evaluationOutcome.keyAreas.map((area, i) => (
            ...
            <div className="bg-violet-50 border border-violet-100 rounded-lg px-4 py-3">
              <h4 className="text-xs font-semibold text-violet-700 uppercase tracking-wider mb-1">Overarching Therapy Goal</h4>
              <p className="text-sm text-violet-900 font-medium leading-relaxed italic">"{patient.evaluationOutcome.overarchingGoal}"</p>
            </div>
          </div>
        </div>
      )}
